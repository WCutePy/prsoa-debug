...
## What is an overlay?
Like many other embedded systems, the Nintendo DS is quite memory-constrained (it only has 4 MiB of RAM). In order to reduce RAM usage, NDS games like _Shadows of Almia_ use a technique called [overlaying](https://en.wikipedia.org/wiki/Overlay_(programming)).
...
In SoA, the game can has 3 "slots" for loading overlays, which means there can be a maximum of 3 overlays loaded at once (this may or may not be general to all NDS games; I'm not sure). You can read more about how this works in [`arm9.yml`](../symbols/arm9.yml), in the descriptions of the `LOADED_OVERLAY_GROUP_*` symbols. You can also look at the values at `LOADED_OVERLAY_GROUP_*` in a memory viewer if you ever need to determine exactly which overlays are loaded at a certain point in the game.
