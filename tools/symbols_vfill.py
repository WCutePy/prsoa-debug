...
addresses in the prsoa-debug symbol tables, for addresses that are known in
...
python3 symbols_vfill.py -f \
    --dir-na </path/to/SoA_NA_unpacked_dir> \
    --dir-eu </path/to/SoA_EU_unpacked_dir>
...
class SymbolTable:
    """A symbol table from prsoa-debug"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fill in missing version-dependent addresses in the prsoa-debug symbol tables."
    )
    for v in offsets.BINARIES:
        parser.add_argument(
            f"--dir-{v.lower()}",
            help=f"data directory for unpacked SoA ({v}) ROM",
        )
    parser.add_argument(
        "-b",
        "--binary",
