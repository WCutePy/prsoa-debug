...
prsoa-debug symbol tables. It will output a list of changes to the symbols in
...
class Symbol:
    """A symbol from the prsoa-debug symbol tables"""
...
class SymbolList(list):
    """List of symbols from the prsoa-debug symbol tables"""
...
class SymbolBlock:
    """
    Block of symbols (with function and data lists) from the prsoa-debug
    symbol tables
    """
...
class SymbolTable:
    """
    prsoa-debug symbol table, including the collapsed contents of any
    nested subregions
    """
...
def print_symbol_diff(
    ...
    *,
    ...
    Args:
        path (Path): path to symbol table file. Must be within the prsoa-debug
            repository
...
def get_modified_paths(base: str, target: Optional[str]) -> List[Path]:
    """
    Get the paths of all modified symbol table files in prsoa-debug between a
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compare Git revisions of the prsoa-debug symbol tables.",
    )
    parser.add_argument(
        "-v",
        "--verbose",
