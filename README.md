## Heads up!
This project is in a ***VERY*** early state. This is directly forked from pmdsky-debug, and there are going to be plenty of _Explorers of Sky_ leftovers in this repo for a while, while we're getting set up.
...
- Symbol tables in the [`symbols/`](symbols) directory state where certain functions and interesting bits of data are located within the SoA binaries. Many of these symbols are accompanied by additional explanation about the inner workings of the game.
...
This repository contains _annotations_ and _tools_ that are meant to accompany analysis of the _Shadows of Almia_ binaries, but does _not_ contain the binaries themselves. **It is up to you to obtain ROMs yourself.** The legal way is to dump the game files directly from a physical _Shadows of Almia_ cartridge that you own.
...
- For SoA debug info, which can be directly imported into various reverse engineering tools. See [Using Debug Info from `prsoa-debug`](docs/using-debug-info.md) for more information.
- Detailed technical documentation for SoA hacking, for both beginners and experienced hackers alike.
- An easy way to share SoA technical research in a standardized format. See [Contributing to `prsoa-debug`](docs/contributing.md) for more information.
- For symbol tables and API headers that can be used to link against the SoA binaries.
...
## Licensing
The overall `prsoa-debug` project is licensed under [GPLv3](LICENSE.txt). However, the [symbols](symbols) and [C headers](headers) are additionally licensed under MIT; see the separate `LICENSE.txt` files in each directory. If you are exclusively using the symbols and/or C headers, you may choose to use them under either GPLv3 or MIT. If you are using `prsoa-debug` as a submodule for your own project purely for the symbols and/or C headers, the dual license likewise applies, provided the symbols and/or C headers are the _only_ part of `prsoa-debug` being used.

## Credits
The `prsoa-debug` project is a direct fork of UsernameFodder's [`pmdsky-debug`](https://github.com/UsernameFodder/pmdsky-debug) project. The entire project structure follows that of `pmdsky-debug`, so major thanks to all contributors to that project; this repository wouldn't exist without you!
