...
## Files
- [`arm9.yml`](arm9.yml) (and subregions within [`arm9/`](arm9/)) contains all symbols in SoA's main ARM9 binary (usually called `arm9.bin`).
- [`arm7.yml`](arm7.yml) contains all symbols in SoA's secondary ARM7 binary (usually called `arm7.bin`).
...
description: |-
    ...
    stack[0]: description of the fifth input parameter
              (SoA uses the stack for parameters after the first 4)
...
## Licensing
The `prsoa-debug` symbol tables are dual-licensed under [GNU GPLv3](../LICENSE.txt) or [MIT](LICENSE.txt). If you are using the symbol tables in your own project, you may choose to use them under either license.
