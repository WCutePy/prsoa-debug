...
struct screen_fade {
    ...
    int8_t maybe_engine_id; // Is consistently 1 or 2 depending on the engine during init. Takes on other values sometimes, like 0x5.
    ...
    void *window_data; // Points to 0x4000040 for engine A. Messing with this makes iris fade square...
    int8_t field15_0x24;
    int8_t field16_0x25;
    bool unk_bool_0x26; // Seem to always be enabled?
    bool unk_bool_0x27; // Seem to always be enabled?
    int8_t screen_stripe_table_1[263][2][4]; // Only the first 192 rows seem to be used? Corresponds with horizontal rows of pixels.
    int16_t screen_stripe_table_2[263][2]; // May be incorrect, Ghidra isn't liking this choice of data type...
};
